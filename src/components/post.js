...
const Post = () => {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver((entries) => {
      ...
      console.log('Sentinel is intersecting:', target.isIntersecting);
  
      if (target.isIntersecting && !loading && hasMore) {
        loadMorePosts(); // Load more posts when the user is near the bottom
      }
    }, { threshold: 0.9 }); // Trigger when 90% of the sentinel is visible
  ...
  }, [loading, hasMore]); // Dependencies to re-run the observer setup when loading or hasMore changes
 ...
 // Fetch and set like count for the specific post
  ...
  return (
    <form onSubmit={handleSubmit} className={`rounded-md flex flex-col gap-4 ${isDarkMode? 'dark-bg' : 'white-bg'} shadow-lg max-w-[30rem] w-full py-1 `}>
      {userData.filter(post=>post?.postResponseDTO?.postVisibility==='PERSONAL')?.map((post) =>{ 
